...
def makeMove(currentState, currentRemark, timelimit):
    ...
    newState = minimax(currentState.board, currentState.whose_move, 2, current_time + timelimit)
    return [[move, newState[0]], newRemark]


def minimax(board, whoseMove, plyLeft, timelimit):
    ...
    if whoseMove == 1:
        ...
        tempWhoseMove = 1
        provisional = 10000000
    while (time.time() + (timelimit / 15000000000) < timelimit):
        for s in generateMove(board, whoseMove, timelimit):
            newVal = minimax(s, tempWhoseMove, plyLeft - 1, timelimit)
...
def generateMove(board, whose_move, timelimit):
    L = []
    while (time.time() + (timelimit / 15000000000) < timelimit):
        for i in range(8):
            for j in range(8):
...
def introduce():
    return "I'm GM. I'm late because I was memorizing my play book."
...
PIECE_EVALUATION = [0, 0, -125, 125, -800, 800, -700, 700, -1000, 1000, -600, 600, -50000, 50000, -900, 900]
...
def imitator_move_capture(current_board, initial_position, final_position):
    ...
    if can_imitator_capture_king(board, initial_position):
        ...
        return pincer_move_capture(board, initial_position, final_position)
    return board
...
def can_imitator_capture_withdrawer(current_board, initial_position, final_position):
    ...
    c1 = initial_column - 1
    c2 = initial_column + 1
    if can_withdrawer_capture(board, initial_position, final_position):
...
def can_withdrawer_capture(current_board, initial_position, final_position):
    board = [r[:] for r in current_board]
    initial_row = initial_position[0]
    initial_column = initial_position[1]
    final_row = final_position[0]
    ...
    if final_row > initial_row and final_column == initial_column and initial_row > 0:  # movement vertically down
        return isEnemy(withdrawer, board[initial_row - 1][initial_column])
        ...
        return isEnemy(withdrawer, board[initial_row][initial_column - 1])
        ...
        return isEnemy(withdrawer, board[initial_row - 1][initial_column - 1])
...
def king_move_capture(current_board, initial_position, final_position):
    ...
    final_row = final_position[0]
    final_column = final_position[1]
    if can_king_move(board, [initial_row, initial_column], [final_row, final_column]):
...
def can_king_move(current_board, initial_position, final_position):
    ...
    final_row = final_position[0]
    final_column = final_position[1]
    if is_adjacent_to_enemy_imitator_which_is_adjacent_to_friendly_freezer(board, initial_position):
...
def king_capture(current_board, initial_position, final_position):
    ...
    if abs(final_row - initial_row) < 2 and abs(final_column - initial_column) < 2 and (
        isEnemy(king, board[final_row][final_column]) or board[final_row][final_column] == 0):
...
def is_adjacent_to_enemy_imitator_which_is_adjacent_to_friendly_freezer(current_board, piece_position):
    board = [r[:] for r in current_board]
    initial_row = piece_position[0]
    initial_column = piece_position[1]
    imitator = 9 # white imitator
    piece = board[initial_row][initial_column]
    if piece % 2 == 1:
        imitator = 8 # black imitator
    r1 = initial_row - 1
    r2 = initial_row + 1
    c1 = initial_column - 1
    c2 = initial_column + 1
    if r1 >= 0 and board[r1][initial_column] == imitator and is_adjacent_to_enemy_freezer(board, [r1, initial_column]):
        return True
    elif r2 <= 7 and board[r2][initial_column] == imitator and is_adjacent_to_enemy_freezer(board, [r2, initial_column]):
        return True
    elif c1 >= 0 and board[initial_row][c1] == imitator and is_adjacent_to_enemy_freezer(board, [initial_row, c1]):
        return True
    elif c2 <= 7 and board[initial_row][c2] == imitator and is_adjacent_to_enemy_freezer(board, [initial_row, c2]):
        return True
    elif r1 >= 0 and c1 >= 0 and board[r1][c1] == imitator and is_adjacent_to_enemy_freezer(board, [r1, c1]):
        return True
    elif r2 <= 7 and c1 >= 0 and board[r2][c1] == imitator and is_adjacent_to_enemy_freezer(board, [r2, c1]):
        return True
    elif r1 >= 0 and c2 <= 7 and board[r1][c2] == imitator and is_adjacent_to_enemy_freezer(board, [r1, c2]):
        return True
    elif r2 <= 7 and c2 <= 7 and board[r2][c2] == imitator and is_adjacent_to_enemy_freezer(board, [r2, c2]):
        return True
    return False
